...
public class JsonHiveSchema  {
  ...
  public static void main( String[] args ) throws Exception {
		 ...
		 * IllegalArgumentException("ERROR: No file specified"); }
		 * 
		 * if (args[0].equals("-h")) { help(); System.exit(0); }
		 */

    StringBuilder sb = new StringBuilder();
    BufferedReader br = new BufferedReader( new FileReader("C:\\Users\\qiadong\\Documents\\My_Received_Files\\data2.json") );
    String line;
    StringBuilder sbs = new StringBuilder();
    while ( (line = br.readLine()) != null ) {
    	String[] strings=line.split(":");
    	String s=strings[0].replace("\"", "").replace("{", "").replace("}", "").replace("]", "").replace("[", "");
    	sbs.append(s);
      sb.append(line).append("\n");
    }
   String dString= sbs.toString().split(",")[0];
    br.close();
